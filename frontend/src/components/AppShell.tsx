      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden relative scrollbar-hide pb-24">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="min-h-full max-w-lg mx-auto w-full px-4 sm:px-0"
          >
            {children}
          </motion.div>
        </AnimatePresence>
...
function NavButton({ icon, label, isActive, onClick }: { icon: React.ReactNode, label: string, isActive: boolean, onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className={`flex flex-col items-center justify-center gap-1 w-20 h-14 rounded-xl transition-all duration-200 active:scale-95 group ${
